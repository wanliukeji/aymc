var load = new Vue({
    ...
    methods: {
        ...
        dele:function (item) {
            ...
            let id = item.id;
            this.$http.post(url, JSON.stringify(id), {emulateJSON: true}).then(function (res) {
                if (res.status == 200) {
                    this.items = res.data.data.records;
                    //异步刷新
                    this.created;
        ...
        edit:function (item) {
            var url = "/api/video/edit";
            this.$http.post(url, JSON.stringify(item), {emulateJSON: true}).then(function (res) {
                if (res.status == 200) {
                    this.items = res.data.data.records;
                    //异步刷新
                    this.created;
